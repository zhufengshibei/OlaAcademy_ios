...
+(UIImage*) squareImage:(UIImage*)image {
    ...
    if (image.size.width > image.size.height) {
        newimg = [image croppedImage:CGRectMake((image.size.width-image.size.height)/2, 0, image.size.height, image.size.height)];
    }else{
        newimg = [image croppedImage:CGRectMake(0, (image.size.height-image.size.width)/2, image.size.width, image.size.width)];
    }
    return newimg;
}
