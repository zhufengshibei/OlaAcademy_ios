...
@property (nonatomic, weak) CXPhotoBrowser *photoBrowser;
...
- (void)displayImage
{
    if (_photo && !_photoImageView.image)
    {
		...
		if (img)
        {
			...
			// Setup photo frame
			CGRect photoImageViewFrame = self.bounds;
            ...
            NSString *imageUrl = ((CXPhoto*)_photo).imageUrl;
            
            if (self.isFromAlbum && (!imageUrl || [imageUrl isEqualToString:@""])) {
                _photoImageView.image = img;
                return;
            }
            // Set image
            //判断本地是否存在当前图片大图的NSData
            
            _photoImageView.hidden = NO;
            
            UIActivityIndicatorView *activityView = (UIActivityIndicatorView*)[_photoImageView viewWithTag:9001];
            if (!activityView) {
                CGFloat width = [UIScreen mainScreen].bounds.size.width;
                CGFloat height = [UIScreen mainScreen].bounds.size.height;
                
                activityView = [[UIActivityIndicatorView alloc] initWithFrame:CGRectMake((width - 40)/2, (height - 40)/2, 40, 40)];
                activityView.tag = 9001;
                [activityView setActivityIndicatorViewStyle:UIActivityIndicatorViewStyleWhiteLarge];
                activityView.backgroundColor = [UIColor clearColor];
                [_photoImageView addSubview:activityView];
            }
            ...
            UIImage *img = ((CXPhoto*)_photo).underlyingImage;
            
            if (imageUrl) {
                __weak __typeof__(self) weakSelf = self;
                
                [_photoImageView sd_setImageWithURL:[NSURL URLWithString:imageUrl] placeholderImage:img completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
                    if (error) {
                        ((CXPhoto*)weakSelf.photo).currentImge = image;
