...
{    
    ...
    CGRect frame = CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, yyy);
...
- (void)resetCustomlizeBrowserToolBarView
{
    ...
    {
        /**
         *  返回自定义browserToolBarView 不为空 就不要设置Frame 啦
         */
	...
	for (NSUInteger index = (NSUInteger)iFirstIndex; index <= (NSUInteger)iLastIndex; index++) {
		if (![self isDisplayingPageForIndex:index]) {
			...
			if (!page) {
                ...
                page.isFromAlbum = self.isFromAlbum;
			}
...
//    frame.origin.y = browserNavigationBarView.frame.origin.y + browserNavigationBarView.frame.size.height + ios7 ? 20 : 0;
...
- (void)currentPageDidUpdated
{
    ...
    if (_shouldUseDefaultUINavigationBar)
    {
        self.title = [NSString stringWithFormat:@"%ld of %ld", _currentPageIndex+1, _photoCount];
    }
}
...
{
    if (_photos.count > _currentPageIndex) {
        CXPhoto *photo = _photos[_currentPageIndex];
        return [[UIImageView alloc] initWithImage:photo.underlyingImage];
    }
    return nil;
}
